...
$ret1=mysqli_query($con,"SELECT users.*, province.*, state.*, city.* FROM users INNER JOIN province ON users.province=province.id INNER JOIN state ON users.State=state.id INNER JOIN city ON users.city=city.id WHERE users.id ='".$_GET['id']."' and users.province = '".$_SESSION['province']."'");
