...
<tbody>
<?php 
$query=mysqli_query($con,"select tblcomplaints.*,users.fullName as name from tblcomplaints join users on users.id=tblcomplaints.userId where tblcomplaints.status is null and tblcomplaints.province ='".$_SESSION['province']."'");
while($row=mysqli_fetch_array($query))
{
?>	
										<tr>
