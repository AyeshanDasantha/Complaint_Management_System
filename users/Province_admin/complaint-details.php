...
<?php $st='closed';
$query=mysqli_query($con,"select tblcomplaints.*,complaintstype.complaintType as cmptype,state.stateName as state,users.fullName as name,category.categoryName as catname from tblcomplaints join users on users.id=tblcomplaints.userId join category on category.id=tblcomplaints.category join state on state.id = tblcomplaints.state join complaintstype on complaintstype.id=tblcomplaints.complaintType where tblcomplaints.province='".$_SESSION['province']."' and tblcomplaints.complaintNumber='".$_GET['cid']."'");
while($row=mysqli_fetch_array($query))
{
