...
<?php
$id=intval($_GET['id']);
$query=mysqli_query($con,"select appealcomplaint.*,users.fullName,users.userEmail,province.provinceName from appealcomplaint INNER JOIN users on users.id=appealcomplaint.UserID INNER JOIN province on province.id=appealcomplaint.province where appealcomplaint.id='".$_GET['id']."' and appealcomplaint.province = '".$_SESSION['province']."'");
while($row=mysqli_fetch_array($query))
{
...
<!-- dadz -->
										...
										<td><b>Payament Status</b></td>
											...
											if($paystat == 0)
											{?>
											  <td  style="color: red"> Pending Payment</a></td>
											<?php }
											else
											{?>
										 ...
										 <?php } ?>
											...
											{?>
												<td><b>Payament Date</b></td>
												<td colspan="4"><?php echo $paydate;?></td>
										 <?php } ?>
											</td>
										</tr>
												<tr>
