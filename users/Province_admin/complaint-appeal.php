...
<tbody>
<?php 
$query=mysqli_query($con,"select appealcomplaint.*,users.fullName,province.provinceName from appealcomplaint INNER JOIN users on users.id=appealcomplaint.UserID INNER JOIN province on province.id=appealcomplaint.province where appealcomplaint.province = '".$_SESSION['province']."'");
$cnt=1;
while($row=mysqli_fetch_array($query))
{
?>	
